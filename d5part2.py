...
for line in f:
    ...
    for crate in range(0, cratesMoving):  # Number of crates 3 would be equivalent to 0 to 2
        ...
        print("Items on Crane Stack prior to Move: " + str(stackHeldbyCrane))
        stackHeldbyCrane.append(itemMovingFromStack)
        print("Items on Crane Stack after Move: " + str(stackHeldbyCrane))
        allStacks[fromStack-1].pop()
        print("Items remaining on From Stack (" + str(fromStack) + ") after removal: " + str(allStacks[fromStack-1]))
    print("All items on Crane Stack: " + str(stackHeldbyCrane))
    print("--")
    # Crates on Crane to To Stack  
    for crate in range(0, len(stackHeldbyCrane)):
        #print(crate)  
        itemsOnCraneStack = len(stackHeldbyCrane)
        print("Items on Crane Stack: " + str(itemsOnCraneStack))
        itemMovingFromCrane = stackHeldbyCrane[itemsOnCraneStack-1]
        print("Item Moving from Crane to Stack (" + str(toStack) +"): " + itemMovingFromCrane)
        # Move item to stack
        allStacks[toStack-1].append(itemMovingFromCrane)
        print("Items on To Stack (" + str(toStack) + ") after Move: " + str(allStacks[toStack-1]))
        # Remove item from Crane    
        stackHeldbyCrane.pop()
        print("Items remaining on Crane after removal: " + str(stackHeldbyCrane))
