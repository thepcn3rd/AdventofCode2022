...
def outputGrid(maxX, maxY, db, cur):
    ...
    for rowY in range(0, maxY+1):
        ...
        # Output the grid with the best hidden trees
        print(outputString)
...
def calcScenicScore(maxX, maxY, db, cur):
    #print("\n")
    #sql = "SELECT uniqueID, colX, rowY, heightTree FROM treeHouse WHERE visibleAll='Hidden'"
    sql = "SELECT uniqueID, colX, rowY, heightTree FROM treeHouse"
    #print(sql)
    results = db.execute(sql)
    f = open("scenic.debug", "w")
    for row in results:
        uID = row[0]
        minX = 0
        minY = 0
        ...
        y = row[2]
        heightTree = row[3]
        ### Check scenic score to the North to the border
        scenicScoreNorth = 0
        for neighborY in range(y-1, minY-1, -1):
            #print("Checking neighbor: (" + str(x) + "," + str(neighborY) + ")")
            neighborTreeHeight = checkNeighborHeight(x, neighborY, db, cur)
            if heightTree > neighborTreeHeight:
                scenicScoreNorth += 1
            elif heightTree <= neighborTreeHeight:
                scenicScoreNorth += 1
                break
        ### Check scenic score to the South to the border # Fixed
        scenicScoreSouth = 0
        for neighborY in range(y+1, maxY+1):
            neighborTreeHeight = checkNeighborHeight(x, neighborY, db, cur)
            #print("Checking neighbor to South: (" + str(x) + "," + str(neighborY) + ") Tree Height: " + str(neighborTreeHeight))
            if heightTree > neighborTreeHeight:
                scenicScoreSouth += 1
            elif heightTree <= neighborTreeHeight:
                scenicScoreSouth += 1
                break
        ### Check scenic score to the East to the border
        scenicScoreEast = 0
        for neighborX in range(x+1, maxX+1):
            #print("Checking neighbor to East: (" + str(neighborX) + "," + str(y) + ")")
            neighborTreeHeight = checkNeighborHeight(neighborX, y, db, cur)
            if heightTree > neighborTreeHeight:
                scenicScoreEast += 1
            elif heightTree <= neighborTreeHeight:
                scenicScoreSouth += 1
                break
        ### Check scenic score to the West to the border
        scenicScoreWest = 0
        for neighborX in range(x-1, minX-1, -1):
            #print("Checking neighbor: (" + str(x) + "," + str(neighborY) + ")")
            neighborTreeHeight = checkNeighborHeight(neighborX, y, db, cur)
            if heightTree > neighborTreeHeight:
                ...
                scenicScoreWest += 1
                break
        # Total Scenic Score is 0 is on the edge
        edge = False
        totalScenicScore = scenicScoreWest * scenicScoreEast * scenicScoreNorth * scenicScoreSouth
        if x == 0 or y == 0 or x == maxX or y == maxY:
            totalScenicScore = 0
            edge = True
        scenicScoreOutput = "C: (" + str(x) +", " + str(y) + ") Tree Height: " + str(heightTree) + " SN: " + str(scenicScoreNorth) + " SW: " + str(scenicScoreWest) + " SE: " + str(scenicScoreEast) + " SS: " + str(scenicScoreSouth)
        #print(scenicScoreOutput)
        f.write(f + "\n")
        #if edge == False:
        #    print("Scenic Score: " + str(totalScenicScore))
        #else:
        #    print("Scenic Score: " + str(totalScenicScore) + " -- Edge --")
        #print("---")
        sqlUpdate = "UPDATE treeHouse SET scenicScore=" + str(totalScenicScore) + " WHERE uniqueID=" + str(uID)
        db.execute(sqlUpdate)
        db.commit()
    f.close()
...
def main():
    ...
    # Part 2 - If the tree is hidden calculate the scenic score...
    calcScenicScore(maximumX, maximumY, dbConnection, dbCursor)
    sql = "SELECT max(scenicScore) FROM treeHouse"
    results = dbConnection.execute(sql)
    for row in results:
        maxScenicScore = row[0]
    print("Part 2: Max Scenic Score is " + str(maxScenicScore))
    dbConnection.close()
